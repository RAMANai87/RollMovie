...
fun MovieScreen(
    ...
    Column {
        ...
        SliderImagesView(
            titleText = "Popular Movie",
            data = dataPopular.value!!,
            onSeeAllClicked = { onSeeAllClicked.invoke(AppConstants.POPULAR_MOVIE) }) {
            onItemClicked.invoke(it)
        }

        MinimalLazyRow(
            titleText = "TopRated Movie",
            data = dataTopRated.value!!,
            { onSeeAllClicked.invoke(AppConstants.TOP_RATED_MOVIE) }) {
            onItemClicked.invoke(it)
        }

        MinimalLazyRow(
            titleText = "NowPlaying Movie",
            data = dataNowPlaying.value!!,
            { onSeeAllClicked.invoke(AppConstants.NOW_PLAYING_MOVIE) }) {
            onItemClicked.invoke(it)
        }
        ...
        MinimalLazyRow(
            titleText = "Discover Movie",
            data = dataDiscover.value!!,
            { onSeeAllClicked.invoke(AppConstants.DISCOVER_MOVIE) }) {
            onItemClicked.invoke(it)
        }

        MinimalLazyRow(
            titleText = "Trending Movie",
            data = dataTrending.value!!,
            { onSeeAllClicked.invoke(AppConstants.TRENDING_MOVIE) }) {
            onItemClicked.invoke(it)
        }
        ...
        MinimalLazyRow(
            titleText = "UpComing Movie",
            data = dataUpComing.value!!,
            { onSeeAllClicked.invoke(AppConstants.UP_COMING_MOVIE) }) {
            onItemClicked.invoke(it)
        }
